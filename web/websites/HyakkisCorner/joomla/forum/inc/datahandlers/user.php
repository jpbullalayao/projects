...
class UserDataHandler extends DataHandler
{
	...
	*/
    ...
    function verify_twitter()
    {
        ...
        /* Inserts into database as empty entry */
        if(empty($twitter) || my_strtolower($twitter) == '@')
        {
            $twitter = '';
            return true;
        }

        // Does the twitter username start with @?
        if(substr($twitter, 0, 1) != "@")
        {
            // Twitter username doesn't start with @, let's see if the user forgot.
            $twitter = "@".$twitter;
        }
    ...
    function verify_instagram()
    {
        ...
        /* Inserts into database as empty entry */
        if(empty($instagram) || my_strtolower($instagram) == '@')
        {
            $instagram = '';
            return true;
        }
        
        // Does the instagram username start with @?
        if(substr($instagram, 0, 1) != "@")
        {
            // Instagram username doesn't start with @, let's see if the user forgot.
            $instagram = "@".$instagram;
        }
    ...
    function verify_facebook()
    {
        ...
        /* Inserts into database as empty entry */
        if(empty($facebook) || my_strtolower($facebook) == 'http://facebook.com/')
        {
            $facebook = '';
            return true;
        }
    ...
    function verify_youtube()
    {
        $youtube = &$this->data['youtube'];
        
        /* Inserts into database as empty entry */
        if(empty($youtube) || my_strtolower($youtube) == 'http://youtube.com/user/')
        {
            $youtube = '';
            return true;
        }
	...
	function verify_twitch()
    {
        $twitch = &$this->data['twitch'];
        
        /* Inserts into database as empty entry */
        if(empty($twitch) || my_strtolower($twitch) == 'http://twitch.tv/')
        {
            $twitch = '';
            return true;
        }
	...
	*/
	function validate_user()
	{
        ...
        if($this->method == "insert" || array_key_exists('twitch', $user)) {
            $this->verify_twitch();
        }
		
        /* ADDING FOR SOCIAL MEDIA SECTION */
        if($this->method == "insert" || array_key_exists('facebook', $user)) {
            $this->verify_facebook();
        }
        if($this->method == "insert" || array_key_exists('youtube', $user)) {
            $this->verify_youtube();
        }  
        if($this->method == "insert" || array_key_exists('twitter', $user)) {
            $this->verify_twitter();
        }
        if($this->method == "insert" || array_key_exists('instagram', $user)) {
            $this->verify_instagram();
        } 
	...
	*/
	function insert_user()
	{
		...
		$this->user_insert_data = array(
			...
			// JUST ADDED
			//"twitter" => $db->escape_string(htmlspecialchars_uni($user['twitter'])),
	...
	*/
	function update_user()
	{
        ...
        /* ADDED FOR GAMER PROFILES */
        if(isset($user['twitch'])) 
        {
        	$this->user_update_data['twitch'] = $db->escape_string(htmlspecialchars_uni($user['twitch']));
        }
        if(isset($user['xone_gamertag']))
        {
            $this->user_update_data['xone_gamertag'] = $db->escape_string(htmlspecialchars_uni($user['xone_gamertag']));
        }
        if(isset($user['x360_gamertag']))
        {
            $this->user_update_data['x360_gamertag'] = $db->escape_string(htmlspecialchars_uni($user['x360_gamertag']));
        }
        if(isset($user['ps4_psn']))
        {
            $this->user_update_data['ps4_psn'] = $db->escape_string(htmlspecialchars_uni($user['ps4_psn']));
        }
        if(isset($user['ps3_psn']))
        {
            $this->user_update_data['ps3_psn'] = $db->escape_string(htmlspecialchars_uni($user['ps3_psn']));
        }
        if(isset($user['steam_user']))
        {
            $this->user_update_data['steam_user'] = $db->escape_string(htmlspecialchars_uni($user['steam_user']));
        }
        if(isset($user['_3ds']))
        {
            $this->user_update_data['_3ds'] = $db->escape_string(htmlspecialchars_uni($user['_3ds']));
        }
        if(isset($user['wiiu_user']))
        {
            $this->user_update_data['wiiu_user'] = $db->escape_string(htmlspecialchars_uni($user['wiiu_user']));
        }
        if(isset($user['wii_num']))
        {
            $this->user_update_data['wii_num'] = $db->escape_string(htmlspecialchars_uni($user['wii_num']));
        }
        /* END OF GAMER PROFILES */
        
        
        /* ADDED FOR SOCIAL MEDIA */
        if(isset($user['facebook']))
        {
            $this->user_update_data['facebook'] = $db->escape_string(htmlspecialchars_uni($user['facebook']));
        }
        if(isset($user['youtube']))
        {
            $this->user_update_data['youtube'] = $db->escape_string(htmlspecialchars_uni($user['youtube']));
        }
        if(isset($user['twitter']))
        {
            $this->user_update_data['twitter'] = $db->escape_string(htmlspecialchars_uni($user['twitter']));
        }
        if(isset($user['instagram']))
        {
            $this->user_update_data['instagram'] = $db->escape_string(htmlspecialchars_uni($user['instagram']));
        }
        if(isset($user['vine']))
        {
            $this->user_update_data['vine'] = $db->escape_string(htmlspecialchars_uni($user['vine']));
        }
        /* END OF SOCIAL MEDIA */
