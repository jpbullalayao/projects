...
if($mybb->input['action'] == "profile")
{
    ...
    $lang->gaming_information = $lang->sprintf($lang->gaming_information, $memprofile['username']);

    // ADDED FOR SOCIAL MEDIA
    $lang->social_media = $lang->sprintf($lang->social_media, $memprofile['username']);
	...
	if($mybb->user['uid'] != $memprofile['uid'] && $mybb->user['uid'] != 0)
	{
		...
		{
			$buddy_options = "<br /><a class=\"buddy\" href=\"./usercp.php?action=do_editlists&amp;delete={$mybb->input['uid']}&amp;my_post_key={$mybb->post_code}\"><img src=\"{$theme['imgdir']}/remove_buddy.gif\" alt=\"{$lang->remove_from_buddy_list}\" /> {$lang->remove_from_buddy_list}</a>";
		}
		else
		{
			$buddy_options = "<br /><a class=\"buddy\" href=\"./usercp.php?action=do_editlists&amp;add_username=".urlencode($memprofile['username'])."&amp;my_post_key={$mybb->post_code}\"><img src=\"{$theme['imgdir']}/add_buddy.gif\" alt=\"{$lang->add_to_buddy_list}\" /> {$lang->add_to_buddy_list}</a>";
		}

		$ignore_list = explode(',', $mybb->user['ignorelist']);
		if(in_array($mybb->input['uid'], $ignore_list))
		{
			$buddy_options .= "<br /><a class=\"buddy\" href=\"./usercp.php?action=do_editlists&amp;manage=ignored&amp;delete={$mybb->input['uid']}&amp;my_post_key={$mybb->post_code}\"><img src=\"{$theme['imgdir']}/remove_ignore.gif\" alt=\"{$lang->remove_from_ignore_list}\" /> {$lang->remove_from_ignore_list}</a>";
		}
		else
		{
			$buddy_options .= "<br /><a class=\"buddy\" href=\"./usercp.php?action=do_editlists&amp;manage=ignored&amp;add_username=".urlencode($memprofile['username'])."&amp;my_post_key={$mybb->post_code}\"><img src=\"{$theme['imgdir']}/add_ignore.gif\" alt=\"{$lang->add_to_ignore_list}\" /> {$lang->add_to_ignore_list}</a>";
		}
	}
